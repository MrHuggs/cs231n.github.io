...
import numpy as np
import sys
sys.path.append('/mnt/c/Users/Dude/Documents/JupyterNotebooks/assignment1')
...
for k in k_choices:
    ...
    for i in range(num_folds):
        xl = X_train_folds[:i] + X_train_folds[i + 1:]
        yl = y_train_folds[:i] + y_train_folds[i + 1:]

        classifier = KNearestNeighbor()
        classifier.train(np.concatenate(xl), np.concatenate(yl))

        y_test_pred = classifier.predict(X_train_folds[i], k)
        num_correct = np.sum(y_test_pred == y_train_folds[i])
        accuracy = float(num_correct) / (num_training / num_folds)
        print(k,i, num_correct, accuracy)
        lac.append(accuracy)
    
    k_to_accuracies[k] = lac
...
# 
# $\color{blue}{\textit Your Answer:}$
# 1 - false - there is no linear seperator
# 2 - yes, the training error will be 0
# 3 - no
# 4 - yes
# 
# $\color{blue}{\textit Your Explanation:}$
